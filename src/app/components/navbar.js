...
export default function Navbar() {
  ...
  return (
    <>
      <div className="h-18 h-fit text-xl flex items-center justify-between">
        ...
        <div className="flex items-center justify-center max-sm:hidden max-md:inline max-md:flex max-md:justify-center">
          <div className="flex items-center capitalize">
            <Link href="/#about" className="p-2 px-4 m-3 cursor hover:shadow-lg hover:shadow-gray-300 hover:rounded-full">
              about
            </Link>
            <Link href="/#projects" className="p-2 px-4 m-3 cursor hover:shadow-lg hover:shadow-gray-300 hover:rounded-full">
              projects
            </Link>
          </div>
        </div>
        <div className="text-4xl sm:hidden max-sm:block">
          {!isClick && (
            <i
              class="fa-solid fa-bars absolute right-5 top-5"
              onClick={isclicked}
            ></i>
          )}
          {isClick && (
            <i
              className="fa-solid fa-xmark absolute right-5 top-5"
              onClick={isclicked}
            ></i>
          )}
        </div>
      ...
      {isClick && (
        <div className="absolute w-screen md:hidden bg-[#E5E8E8] text-xl">
